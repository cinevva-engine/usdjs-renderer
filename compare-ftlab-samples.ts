...
type Args = {
    // Root of packages/usdjs (where test/corpus/... lives)
    usdjsRoot: string;
...
function parseArgs(argv: string[]): Args {
    ...
    return { usdjsRoot, sampleRel, width, height, compose };
}
...
async function main() {
    ...
    const sampleRootAbs = path.join(
        usdjsRootAbs,
        'test/corpus/external/ft-lab-sample-usd/sample_usd-main',
    );
    ...
    for (const m of filtered) {
        ...
        // Current viewer harness loads text layers via loadTextFiles (USDA/USD/TXT) and resolves textures via /__usdjs_corpus.
        // Binary USD samples (e.g. .usdc) are not supported by this pipeline yet.
        ...
        try {
            // Clean tab per sample to avoid state leaks (camera/scene/material caches) across renders.
            ...
            // Load text files for composition/entry resolution.
